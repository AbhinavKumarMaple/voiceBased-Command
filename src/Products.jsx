...
const Products = () => {
    ...
    try {
      ...
      newData.forEach((newProduct) => {
        ...
        if (oldProduct) {
          ...
          if (oldProduct.inventory !== newProduct.inventory) updatedProductFields.inventory = true;
          if (oldProduct.lastModified !== newProduct.lastModified) updatedProductFields.lastModified = true;
  ...
  const getHighlightStyle = (productId, field) => {
    return updatedFields[productId] && updatedFields[productId][field] ? { color: 'red' } : {};
  };
        ...
        <tbody>
          {data.map((product) => (
            <tr key={product._id}>
              ...
              <td style={getHighlightStyle(product._id, 'inventory')}>{product.inventory}</td>
